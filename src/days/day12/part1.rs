...
fn internal_solve(path: &str) -> u32 {
    ...
    let start = graph.get_start_node();

    let s = &*start.borrow();

    let shortest_path: Option<(Vec<Node>, u32)> = dijkstra(s, |x| x.get_successors(), |x| x.is_end);
    let result = shortest_path.unwrap().1;
    result
}
